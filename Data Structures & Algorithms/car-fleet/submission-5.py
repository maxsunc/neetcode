class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        while stack:
            entry = stack[-1]
            intersectionPlace = -1
            if curEntry[1] != entry[1]:
            # case 1: entry catches up to curEntry (pop it off)
                intersectionPlace = ((entry[1] * curEntry[0] - curEntry[1] * entry[0]) / (-curEntry[1] + entry[1]))
            if target >= intersectionPlace and intersectionPlace >= entry[0]:
                print(f"at {intersectionPlace} {entry} catches up to {curEntry}")
