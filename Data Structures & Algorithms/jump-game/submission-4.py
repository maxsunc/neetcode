...
class Solution:
    def canJump(self, nums: List[int]) -> bool:
        ...
        # greedy: Similiar to bottom up dp????
        if len(nums) == 1:
            return True
        goal = len(nums)-1
        for i in range(len(nums)-2, -1,-1):
            # see if we can reach goal from here if we an then update goal
            jumpLength = nums[i]
            if i + jumpLength >= goal:
                goal = i # we can make it to the goal here so thisi s our new goal
        
        return goal == 0 # if we mae it at index 0 then that means goal = 0
